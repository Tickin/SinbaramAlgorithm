...
void multi(unsigned long long int t[4], unsigned long long int m[4]) {
    ...
    m0 = ( t[0]*m[0] + t[1]*m[2] ) % 1000000007;
    m1 = ( t[0]*m[1] + t[1]*m[3] ) % 1000000007;
    m2 = ( t[2]*m[0] + t[3]*m[2] ) % 1000000007;
    m3 = ( t[2]*m[1] + t[3]*m[3] ) % 1000000007;
...
void square(unsigned long long int m[4]) {
    ...
    m0 = ( m[0]*m[0] + m[1]*m[2] ) % 1000000007;
    m1 = ( m[0]*m[1] + m[1]*m[3] ) % 1000000007;
    m2 = ( m[2]*m[0] + m[3]*m[2] ) % 1000000007;
    m3 = ( m[2]*m[1] + m[3]*m[3] ) % 1000000007;
